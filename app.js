window.addEventListener("load", () => {
    ...
    async function getLocationCoordinates() {
        ...
        try {
            ...
            const GEO_URL = `https://api.openweathermap.org/geo/1.0/direct?q=${cityName},${countryCode}&limit=1&appid=${AP1_KEY}`

          await fetch(GEO_URL).then(res => res.json()).then(results => {
        ...
        async function getWeatherDetails(cityName, fullName, countryCode, lat, lon) {
            try {
                ...
                fetch(weatherUrl_API).then(res => res.json()).then(result => {
